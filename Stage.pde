class Stage {
  ...
  void update() {
     ...
     textSize(25);
     text("Push Enter ! !",WIDTH/2,HEIGHT/2-100);
    ...
    case 3:
      ...
      if(clear){
        ...
        if(keyPressed){
          if(key == 'N' || key == 'n') mode++;
          if(key == 'T' || key == 't'){ scene = mode = 0; clear = false;}
        }
      }
      else{
...
boolean judge(float a,float b,float c, float d,float r){
  r /=2;
...
//
//float di(float a,float b,float c, float d,float r){
